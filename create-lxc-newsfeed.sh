#!/usr/bin/env bash
#
# create-lxc-newsfeed.sh — LXC Ubuntu 24.04 + newsfeed (agrégateur de veille
# tech & science, Node 22 + Fastify + SQLite, repo tla1852/newsfeed)
#
...
# - Puis : Node 22 (NodeSource), clone du repo newsfeed (PAT demandé),
#          build, service systemd (voir homelab/newsfeed/setup-app.sh)
#
# Usage (sur le noeud Proxmox, en root) :
#   bash <(curl -fsSL https://raw.githubusercontent.com/tla1852/proxmox-scripts/main/create-lxc-newsfeed.sh)

set -euo pipefail

# ----- Configuration -----
STORAGE="local-lvm"
DISK_GB="8"            # app + node_modules + SQLite (rétention 90j, qq dizaines de Mo)
BRIDGE="vmbr0"
...
# ----- Questions ----- (défauts skill : newsfeed / 1 / 1024)
read -rp "Nom du container (hostname) [newsfeed] : " CT_NAME; CT_NAME="${CT_NAME:-newsfeed}"
...
# ═════════════════════════════════════════════════════════════════════════════
# COUCHE APPLICATIVE — NEWSFEED (le socle ci-dessus fournit git + user thibault)
# ═════════════════════════════════════════════════════════════════════════════
# Déléguée à homelab/newsfeed/setup-app.sh, relançable seul en cas d'échec :
